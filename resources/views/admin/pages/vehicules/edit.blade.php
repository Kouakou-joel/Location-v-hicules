...
<div class="container">
    ...
    <form action="{{ route('vehicules.update', $vehicule->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="user_id" id="user_id" class="form-select" required>
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->id }}" {{ $user->id == $vehicule->user_id ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="vehicule_id" id="vehicule_id" class="form-select" required>
                ...
                @foreach ($vehicule as $vehicule)
                    <option value="{{ $vehicule->id }}" {{ $vehicule->id == $vehicule->vehicule_id ? 'selected' : '' }}>
                        {{ $vehicule->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="images" id="images" class="form-control"  required>
        ...
        <button type="submit" class="btn btn-primary">Mettre à jour le vehicule</button>
